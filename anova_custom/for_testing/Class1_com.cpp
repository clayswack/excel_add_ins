...
const wchar_t * Class1::anova_custom_help = L"Objective: Export multcompare figure functionality to Excel so it can be used by non-MATLAB end users\
...
std::map<std::string, const wchar_t*> Class1::createHelpMap()
{
  ...
  helpMap["anova_custom"] = anova_custom_help;
  return helpMap;
}

HRESULT __stdcall Class1::anova_custom(/*[in]*/VARIANT x, /*[in]*/VARIANT y) {
  return( CallComFcn( "anova_custom", 0, 0, 2, &x, &y ) );
}
